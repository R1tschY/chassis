...
pub fn codegen_injectfns(
    ...
    for input in &sig.inputs {
        ...
        let tokens = if let Some(annotation) = get_annotation_attribute(&input.attrs) {
            ...
            quote! {
                dyn_chassis::TypedKey::< <#ty as dyn_chassis::ResolveInto>::Item >
                    ::new_with_annotation(#expr)
            }
        } else {
            quote! { dyn_chassis::TypedKey::< <#ty as dyn_chassis::ResolveInto>::Item >::new() }
        };
    ...
    let code_metafn = quote_spanned! {span=>
        pub fn #metafn_name(__binder__: &mut dyn_chassis::Binder) {
            #[allow(unused_imports)] use dyn_chassis::Named;

            let mut binding = __binder__.bind::<#rty_token>();
            #annotation
            binding.#factory_ident(
                    Self::#injectfn_name,
                    dyn_chassis::meta::InjectionPoint::for_module_function(
                        #userfn_name_str,
                        &[ #(#dep_keys.into()),* ],
                )
            )
    ...
    let code_injectfn = if return_self {
        quote! {
            pub fn #injectfn_name(__sl__: &dyn_chassis::Injector) -> Self {
        ...
        let fn_sig = quote! { pub fn #injectfn_name(__sl__: &dyn_chassis::Injector) };
